...
app.config['MYSQL_USER'] = 'sql6490620'
app.config['MYSQL_PASSWORD'] = 'CNJhfBUx5v'
app.config['MYSQL_DB'] = 'sql6490620'
mysql = MySQL(app)
...
def signin() :
	...
	# getting info from signin form
		...
		cur.execute("SELECT * FROM jobseeker WHERE (email, password) = (%s, %s) ", (email, password))
...
def signup() :
	if request.method == 'POST':
		...
		cur = mysql.connection.cursor()
		cur.execute("SELECT email FROM jobseeker WHERE email = \"{}\"".format(email))
...
def profile() :
	if "user" in session:
		cur = mysql.connection.cursor()
		cur.execute("SELECT jobseeker_id FROM jobseeker WHERE email = \"{}\"".format(session['email']))
		jobseeker_id = cur.fetchone()
		cur.execute("SELECT job.job_title, job.job_type, company.name, company.location, job.job_salary, job.job_id FROM job INNER JOIN company ON job.company_id = company.company_id WHERE job.job_id in (SELECT job_id FROM apply WHERE jobseeker_id=" + str(jobseeker_id[0]) + ")")
		applied_jobs = cur.fetchall()
		cur.execute("SELECT * FROM profile WHERE jobseeker_id=" + str(jobseeker_id[0]))
	...
	if "user" in session:
		...
		if request.method == 'POST':
			...
			if profile_data:
				...
				query = "UPDATE profile SET college = \"{}\", department = \"{}\", education = \"{}\", resume=\"{}\" WHERE jobseeker_id=".format(college, dept, education, filename) + str(jobseeker_id[0])
				cur.execute(query)
				mysql.connection.commit()
			else:
				if filename :
					...
					with open(path, 'wb') as file:
						file.write(resume.read())
				cur.execute(f"INSERT INTO profile(college, department, education, resume, jobseeker_id) VALUES (\"{college}\", \"{dept}\", \"{education}\", \"{filename}\"," + str(jobseeker_id[0]) + ")")
				mysql.connection.commit()
			cur.close()
...
def apply():
	if "user" in session:
		if request.method == 'POST':
			...
			cur.execute("SELECT jobseeker_id FROM jobseeker WHERE email = \"{}\"".format(session['email']))
			jobseeker_id = cur.fetchone()	
			cur.execute("SELECT * FROM apply WHERE (jobseeker_id, job_id) = (\"{}\", \"{}\")".format(str(jobseeker_id[0]), job_id))
			applied = cur.fetchall()
			if len(applied) == 0:
				cur.execute("INSERT INTO apply VALUES (\"{}\", \"{}\")".format(str(jobseeker_id[0]), job_id))
				mysql.connection.commit()
			cur.close()
		return redirect("/")
	else:
...
def interview() :
	if "user" in session:
		...
		cur.execute("SELECT jobseeker_id FROM jobseeker WHERE email = \"{}\"".format(session['email']))
		jobseeker_id = cur.fetchone()	
		cur.execute("SELECT * FROM apply INNER JOIN interview ON (apply.jobseeker_id, apply.job_id) = (interview.jobseeker_id, interview.job_id) WHERE interview.jobseeker_id=" + str(jobseeker_id[0]))
		check_apply = cur.fetchall()
		if len(check_apply) > 0:
			cur.execute("SELECT interview.jobseeker_id, job.job_title, company.name, interview.date, interview.time FROM job INNER JOIN company ON job.company_id = company.company_id INNER JOIN interview ON interview.job_id = job.job_id WHERE interview.jobseeker_id=" + str(jobseeker_id[0]) + " AND interview.job_id IN (SELECT apply.job_id FROM apply INNER JOIN interview ON (apply.jobseeker_id, apply.job_id) = (interview.jobseeker_id, interview.job_id) WHERE interview.jobseeker_id=" + str(jobseeker_id[0]) +")")
...
def results() :
	if "user" in session:
		...
		cur.execute("SELECT jobseeker_id FROM jobseeker WHERE email = \"{}\"".format(session['email']))
		jobseeker_id = cur.fetchone()	
		cur.execute('SELECT * FROM apply INNER JOIN result ON (apply.jobseeker_id, apply.job_id) = (result.jobseeker_id, result.job_id) WHERE result.jobseeker_id=' + str(jobseeker_id[0]))
...
def account() :
	if "user" in session:
		...
		cur.execute("SELECT jobseeker_id FROM jobseeker WHERE email = \"{}\"".format(session['email']))
		jobseeker_id = cur.fetchone()
		cur.execute("SELECT * FROM jobseeker WHERE jobseeker_id = " + str(jobseeker_id[0]))
...
def accountSummary() :
	if "user" in session:
		...
		cur.execute("SELECT jobseeker_id FROM jobseeker WHERE email = \"{}\"".format(session['email']))
		jobseeker_id = cur.fetchone()	
		cur.execute('SELECT count(job_id) FROM apply GROUP BY jobseeker_id HAVING jobseeker_id  = ' + str(jobseeker_id[0]))
		applied = cur.fetchone()
		cur.execute('SELECT count(job_id) FROM result GROUP BY jobseeker_id HAVING jobseeker_id = ' + str(jobseeker_id[0]))
		results = cur.fetchone()
		cur.execute('SELECT count(job_id) FROM interview GROUP BY jobseeker_id HAVING jobseeker_id = ' + str(jobseeker_id[0]))
...
def changePassword():
	if "user" in session:
		if request.method == "POST" :
			...
			cur.execute("SELECT jobseeker_id FROM jobseeker WHERE email = \"{}\"".format(session['email']))
			jobseeker_id = cur.fetchone()
			cur.execute(f"UPDATE jobseeker SET password = \"{password}\" WHERE jobseeker_id = " + str(jobseeker_id[0]))
			mysql.connection.commit()
			cur.close()
...
def editAccount():
	if "user" in session:
		if request.method == "GET":
			cur = mysql.connection.cursor()
			cur.execute("SELECT * FROM jobseeker WHERE email = \"{}\"".format(session['email']))
			...
			if(not phone_number.isnumeric()):
				flash("Enter only digits [0-9]", "warning")
				return redirect(request.url)
			cur.execute(f"UPDATE jobseeker SET first_name = \"{first_name}\", last_name = \"{last_name}\", phone_number = {phone_number}, email = \"{email}\" WHERE jobseeker_id = " + str(jobseeker_id[0]))	
			mysql.connection.commit()
			session['email'] = email 
			cur.close()
			return redirect("/account")
	...
	if "user" in session and session["email"] == "admin@hireit.com":
		if request.method == "POST" :
			...
			if exists :
				...
				return redirect(request.url)
			cur.execute(f"INSERT INTO company (name, location) VALUE (\"{name}\", \"{location}\")")
			mysql.connection.commit()
			cur.close()
...
def addjob():
	...
	if "user" in session  and session["email"] == "admin@hireit.com":
		if request.method == "POST":
			...
			cur = mysql.connection.cursor()
			cur.execute(f"SELECT company_id FROM company where name LIKE \"%{company_name}%\"")
			...
			cur.execute(f"INSERT INTO job (job_title, job_type, job_description, job_salary, company_id) VALUES (\"{job_title}\", \"{job_type}\", \"{job_description}\", {salary}, {company_id})")
			mysql.connection.commit()
			cur.close()
	...
	if "user" in session and session["email"] == "admin@hireit.com":
		if request.method == "POST" :
			...
			if not company_id:
				flash("Add Company before inserting job", "danger")
				return redirect("/addCompany")
			cur.execute("SELECT job_id FROM job where company_id =" + str(company_id) + f" AND job_title = \"{job_title}\"")
			job_id = cur.fetchone()
			if job_id != None:		
				job_id = job_id[0]
			if not job_id:
			...
			cur.execute(f"SELECT jobseeker_id FROM jobseeker where email LIKE \"{candidate_email}\"")
			jobseeker_id = cur.fetchone()
			...
			if not jobseeker_id:
				...
				return redirect("/")
			cur.execute(f"INSERT INTO interview (job_id, date, time, jobseeker_id) VALUES ({job_id}, \"{date}\", \"{time}\", {jobseeker_id})")
			mysql.connection.commit()
			cur.close()
	...
	if "user" in session and session["email"] == "admin@hireit.com":
		if request.method == "POST" :
			...
			if not jobseeker_id:
				flash("User does not exist", "danger")
				return redirect("/")
			cur.execute(f"INSERT INTO result (jobseeker_id, job_id, status) VALUES ({jobseeker_id}, {job_id}, {int(status)})")
			mysql.connection.commit()
			cur.close()
			return redirect("/")
